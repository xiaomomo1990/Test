...
BootStrap.layout = {
            ...
            _layout
                .each(function () {
                    var _that = $this.Layout = $(this),
                        _north = $this.Layout._north = _that.children("div[region='north']"),
                        _south = $this.Layout._south = _that.children("div[region='south']"),
                        _center = $this.Layout._center = _that.children("div[region='center']"),
                        _east = $this.Layout._east = _that.children("div[region='east']"),
                        _west = $this.Layout._west = _that.children("div[region='west']");
                    ...
                    $this._dragArea = {
                        ...
                        right: { x1: ($this._opt.layoutWidth - 10) * 0.75 + 5, x2: ($this._opt.layoutWidth - 10) * 0.75 + 10, y1: ($this._opt.layoutHeight - 10) * 0.25 + 5, y2: ($this._opt.layoutHeight - 10) * 0.75 + 5 }
                    };
                    ...
                    //初始化五块区域的长和宽，位置
                    $this._layout(_north, _south, _center, _east, _west);
    ...
    //绘制五块区域的长和宽，位置
    _layout: function (_north, _south, _center, _east, _west) {
        var $this = this;
        ...
        //储存五个区域初始的长和宽
        $this._size = {
            north: [$this._dragArea.top.x2, $this._dragArea.top.y1],
            south: [$this._dragArea.bottom.x2, $this._opt.layoutHeight - $this._dragArea.bottom.y2],
            center: [$this._dragArea.right.x1 - $this._dragArea.left.x2, $this._dragArea.bottom.y1 - $this._dragArea.top.y2],
            east: [$this._dragArea.left.x1, $this._dragArea.bottom.y1 - $this._dragArea.top.y2],
            west: [$this._opt.layoutWidth - $this._dragArea.right.x2, $this._dragArea.bottom.y1 - $this._dragArea.top.y2]
        };
        //储存五个区域初始的位置(坐标x,y值)
        $this._location = {
            north: [0, 0],
            south: [0, $this._dragArea.bottom.y2],
            center: [$this._dragArea.left.x2, $this._dragArea.top.y2],
            east: [0, $this._dragArea.top.y2],
            west: [$this._dragArea.right.x2, $this._dragArea.top.y2]
        };
    ...
    _dragaaArea: function (_that, _lsp) {
        var $this = this;
        _that.bind("mousemove", function (e) {
            ...
            else if (xx >= (30 + 5 / 2) && ($this._opt.layoutWidth - xx >= $this._size.west[0] + 5 + 30) && $this.thisDrag == "left") {
            ...
            else if (xx >= (30 + 5 / 2 + $this._size.east[0]) && ($this._opt.layoutWidth - xx >= 30) && $this.thisDrag == "right") {
                $this._dragArea.right.x1 = xx - 5 / 2;
                $this._dragArea.right.x2 = xx + 5 / 2;
                _lsp.css({
                    left: xx + "px",
                    top: $this._location.east[1] + "px",
                    width: "5px",
            ...
            $(document).bind("mouseup", function () {
                ...
                //拖动释放，重绘布局
                $this._dragRelease(_lsp, _that, $this);
    ...
    _dragRelease: function (_lspV, _that, $this) {
        ...
        //重绘
        $this._layout($this.Layout._north, $this.Layout._south, $this.Layout._center, $this.Layout._east, $this.Layout._west);
    },
    //拖动Y轴显示
    _dragY: function (_lsp, $this, yy) {
